...
float a_ConnInterval[SIZE_TAB_CONN_INT] = {36, 60}; /* ms ykk 20250923 将连接间隔设置为45ms - 75ms 原来的是50ms - 1000ms */
...
void BLE_SVC_L2CAP_Conn_Update(uint16_t ConnectionHandle)
{
  ...
  /* ykk 20250923  L2CAP_INTERVAL_MIN / MAX 宏直接请求区间。*/
  if (mutex == 1)
  {
    mutex = 0; /* 防止重复发送 */
    uint16_t interval_min = L2CAP_INTERVAL_MIN; /* (单位1.25ms) */
    uint16_t interval_max = L2CAP_INTERVAL_MAX; /* (单位1.25ms) */
    uint16_t peripheral_latency = L2CAP_PERIPHERAL_LATENCY; /* 0 表示无跳过，提高实时性 */
    uint16_t timeout_multiplier = L2CAP_TIMEOUT_MULTIPLIER;  /* 监督超时 5s */
